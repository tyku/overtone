...
- Вход по почте и выданному администратором паролю; у каждого пользователя не более одного незакрытого приёма.
- Пользователь относится к одной клинике и может заполнить ФИО, должность и специализацию.
- Админка создаёт клиники и пользователей, назначает права, блокирует доступ и перевыпускает одноразовый пароль.
...
Сессия хранится на backend; браузер получает только случайный токен в `HttpOnly`
cookie. Приёмы изолированы по текущему пользователю.
...
После первого запуска создайте начального администратора. Команда откажется
работать, если администратор уже существует:

```sh
cd ../local-stack
docker compose exec -T api node dist/admin/bootstrap-admin.js \
  --email admin@example.com --clinic "Название клиники"
```

Команда напечатает пароль один раз и назначит первому администратору старые
приёмы технического пользователя. Откройте `http://localhost:8080/login`,
войдите с выданными данными, затем перейдите в `/admin`.
...
|---|---|---|
| POST | `/api/auth/login` | Вход по `{ "email", "password" }`, создаёт серверную сессию |
| POST | `/api/auth/logout` | Завершить текущую сессию |
| GET | `/api/auth/session` | Текущий пользователь, клиника и права |
| PATCH | `/api/auth/profile` | Изменить опциональные ФИО, должность и специализацию |
| GET/POST | `/api/admin/clinics` | Список и создание клиник |
| GET/POST | `/api/admin/users` | Список и создание пользователей; пароль только в create-response |
| PATCH | `/api/admin/users/{id}` | Клиника, профиль, права и блокировка пользователя |
| POST | `/api/admin/users/{id}/regenerate-password` | Перевыпустить и один раз вернуть новый пароль |
