...
export class RecordingsGateway implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleDestroy() {
    this.logger.log(
      `Stopping WebSocket recording gateway; activeSessions=${this.sessions.size}`,
    );
    this.wss?.close();
  }
  ...
  private handleConnection(socket: WebSocket, request: IncomingMessage) {
    ...
    this.logger.log(
      `WebSocket connected: client=${client} activeSessions=${this.sessions.size}`,
    );
        ...
        try {
          ...
          if (message.type === 'start') {
            ...
            session = {
              ...
              stream: createWriteStream(
                join(this.recordingsDir, `${id}.${extension}.part`),
                { flags: 'wx' },
              ),
              chunks: 0,
              closed: false,
          ...
          if (message.type === 'audio-level') {
            if (!session || message.recordingSessionId !== session.id) {
              throw new Error(
                'Audio level does not match the active recording.',
              );
            }
          ...
          if (message.type === 'audio-track-state') {
            if (!session || message.recordingSessionId !== session.id) {
              throw new Error(
                'Audio track state does not match the active recording.',
              );
            }
            this.logger.warn(
              `Microphone track state changed: session=${session.id} state=${message.state ?? 'unknown'}`,
            );
            return;
          }
    ...
    socket.on('error', (error) => {
      this.logger.error(
        `WebSocket error: client=${client} session=${session?.id ?? 'none'}`,
        error.stack,
      );
    });
    ...
    socket.on('close', (code, reason) => {
      ...
      if (session && !session.closed) {
        ...
        void this.closeSession(interruptedSession, false).catch(
          (error: unknown) => {
            this.logger.error(
  ...
  private async closeSession(session: RecordingSession, completed: boolean) {
    ...
    this.logger.log(
      `Closing recording stream: session=${session.id} completed=${completed}`,
    );
