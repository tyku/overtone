...
async function fixture(page: Page) {
  ...
  await page.route('**/api/requests**', async (route) => {
    ...
    else if (url.pathname === '/api/requests') body = { items: state.rows, nextCursor: null };
    else if (url.pathname.endsWith('/complete')) {
      state.uploads.push({ type: req.headers()['content-type'], body: req.postDataBuffer()?.toString() ?? '' });
      if (state.fail) { state.row.status = 'save_failed'; state.row.error = { code: 'REQUEST_FINALIZATION_FAILED', retryAction: 'complete_without_audio', message: 'Retry closing' }; state.row.audioStored = true; status = 503; body = { requestId: id, error: state.row.error }; }
...
test('stop preserves audio; finish sends both parts once and waits without a 30s deadline', async ({ page }) => {
  ...
  expect(state.uploads[0].body).toContain('name="part_1"'); expect(state.uploads[0].body).toContain('name="part_2"'); expect(state.uploads[0].body).toContain('name="manifest"');
  await expect(page.locator('#recorderPanel')).toBeHidden();
  await page.reload(); await expect(page.locator('#waitingPanel')).toBeVisible();
  // Rendered report keeps the same node between polling updates and sanitizes model-produced HTML.
  state.row.status = 'completed';
  await expect(page.locator('#reportContent h1')).toHaveText('Итоговый отчёт', { timeout: 6000 });
  await expect(page.locator('#reportContent script, #reportContent img')).toHaveCount(0);
  expect(await page.evaluate(() => (window as any).injected)).toBeUndefined();
  const downloading = page.waitForEvent('download'); await page.getByRole('button', { name: 'Скачать .md' }).click();
  expect((await downloading).suggestedFilename()).toContain('.md');
  await page.screenshot({ path: '/private/tmp/overtone-api-tests/report-desktop.png', fullPage: true });
});
test('failed finalization survives reload and retries without audio after S3 confirmation', async ({ page }) => {
  const state = await fixture(page); state.fail = true; await start(page);
  await page.getByRole('button', { name: 'Завершить приём', exact: true }).click();
  await expect(page.getByRole('button', { name: 'Повторить сохранение' })).toBeEnabled();
  await expect(page.locator('#requestStatus')).toHaveText('Ошибка сохранения');
  await page.reload(); await expect(page.locator('#recorderPanel')).toBeHidden();
  await expect(page.getByRole('button', { name: 'Продолжить запись' })).toHaveCount(0);
  state.fail = false; await page.getByRole('button', { name: 'Повторить сохранение' }).click();
  await expect(page.locator('#waitingPanel')).toBeVisible();
  expect(state.uploads).toHaveLength(2); expect(state.uploads[1].type).toContain('application/json'); expect(state.uploads[1].body).toBe('{}');
});
test('force-close is explicit, keeps metadata and returns to the history on mobile', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  const state = await fixture(page); state.fail = true; await start(page);
  await page.getByRole('button', { name: 'Завершить приём', exact: true }).click();
  await expect(page.getByRole('button', { name: 'Закрыть без сохранения' })).toBeEnabled();
  ...
  await expect(page.locator('.request-card')).toContainText('Закрыт без сохранения');
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  await page.screenshot({ path: '/private/tmp/overtone-api-tests/history-mobile.png', fullPage: true });
});
